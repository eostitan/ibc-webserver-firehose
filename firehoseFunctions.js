...
console.log("mindreaderApi", process.env.mindreaderApi);
console.log("grpcAddress",grpcAddress);
console.log("firehoseMinBlock",firehoseMinBlock);
...
const getFirehoseClient = () => new bstreamService.BlockStreamV2(
  grpcAddress,
  process.env.grpcInsecure ? grpc.credentials.createInsecure(): grpc.credentials.createSsl(), {
...
const getBlock = req => new Promise((resolve,reject) => {
  ...
  stream.on('error', async error => {
    ...
    // else if (error.code === grpc.status.INTERNAL) console.log("stream closed with code 0");
    else {
      if(req.retries){
...
function checkValidBlockRange(blockNum){
  return new Promise(async resolve=>{
    try{
      ...
      const headBlock = (await axios(`${process.env.mindreaderApi}/v1/chain/get_info`)).data.head_block_num;
...
function formatBFTBlock(number, block){
  ...
  /*  
    {
      "block_id": "055f34a8bd4dccced1488a70bcee2aeeea5856f3c763b1622d8ba814e56127ae",
      "block_num": 90125480,
      "producer": "bp12",
      "header_digest": "ca2afd37bd4dccced1488a70bcee2aeeea5856f3c763b1622d8ba814e56127ae",
      "producer_signatures": [
        "SIG_K1_KhvRGVrcfW62JkqpLRr61BdzRkK9DQoCMuLeqosH8zdKjrHKucexm8wrnyKowZB4aCcnEFXrfa7ZbHddv19XT93BySZh2s"
      ]
    }
  */
